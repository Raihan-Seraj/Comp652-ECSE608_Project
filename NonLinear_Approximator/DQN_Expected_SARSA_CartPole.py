...
def run_episode(env,
    ...
    for i in range(max_step):
        ...
        if len(memory.memory) > batch_size:  # DQN Experience Replay
            ...
            """
            Target for Expected SARSA
            """

            policy = agent.make_epsilon_greedy_policy(q_n_b, env.action_space.n)
            next_action_probs = policy(states_n_b)

            V = np.sum( np.multiply(next_action_probs, q_n_b), axis=1 )
