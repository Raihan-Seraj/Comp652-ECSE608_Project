...
def sarsa_lambda(env, theta, num_episodes, discount_factor=1.0, epsilon=0.1, epsilon_decay=1.0, alpha=0.5, lambda_param=1):
	...
	for i_episode in range(num_episodes):
		...
		for t in itertools.count():
			...
			# # np.array([np.dot(A[:,i], A[:,i]) for i in xrange(A.shape[1])])
			q_values = np.array([np.dot(all_features[:,q], theta[:,q]) for q in xrange(theta.shape[1])])
			...
			next_q_values = np.array([np.dot(all_next_features[:,q], theta[:,q]) for q in xrange(theta.shape[1])])
			next_q_values_state_action = next_q_values[next_action]


			td_target = np.add(reward, discount_factor * next_q_values[action])
			Delta = td_target - q_values[action]
			...
			theta[:, action] += np.multiply(alpha * Delta, eligibility[:, action])
