...
def Q_Sigma_Off_Policy_2_Step(env, theta, num_episodes, discount_factor=1.0, epsilon=0.1, epsilon_decay=1.0):
	...
	for i_episode in range(num_episodes):
		...
		for t in itertools.count():
			...
			# pi_action_probs = policy(state)
			# pi_action = np.random.choice(np.arange(len(pi_action_probs)), p=pi_action_probs)
			# _, pi_reward, done, _ = env.step(pi_action)
			# if done:
			# 	break		
			# stats.episode_rewards[i_episode] += pi_reward
